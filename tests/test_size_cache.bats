...
@test "force pre-flight plus clean measures each distinct root once (e2e)" {
  ...
  mkdir -p \
    ...
    "$stubbin"
  local d
  for d in "$fake_home/.cache" "$fake_home/.cache/pip" "$fake_home/.npm" \
    "$fake_home/.m2/repository" "$fake_home/.gradle/caches" \
    "$fake_home/go/pkg/mod/cache" "$fake_home/.cargo/registry/cache" \
    "$fake_home/.local/share/Trash/files"; do
  ...
  assert_contains "$out" "included in ${fake_home}/.cache"
  ...
  # The nested root was never walked standalone (covered by ~/.cache,
  # then deleted by the caches module before dev_caches could ask).
  if grep -Fxq "${fake_home}/.cache/pip" "$calls"; then
    fail "nested ~/.cache/pip was du-walked as its own root"
  fi

  # Every remaining listed root was measured exactly once.
  local p
  for p in "$fake_home/.cache" "$fake_home/.npm" "$fake_home/.m2/repository" \
    "$fake_home/.gradle/caches" "$fake_home/go/pkg/mod/cache" \
    "$fake_home/.cargo/registry/cache" "$fake_home/.local/share/Trash/files"; do
