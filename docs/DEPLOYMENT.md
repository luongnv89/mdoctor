...
5. Create/push release tag (e.g., `v3.2.1`) — the `Release` workflow
...
```bash
git tag -a v3.2.1 -m "mdoctor v3.2.1"
git push origin v3.2.1

# The Release workflow then creates the release titled "mdoctor v3.2.1"
# with docs/CHANGELOG.md as the notes. Manual equivalent (fallback only):
gh release create v3.2.1 \
  --repo luongnv89/mdoctor \
  --title "mdoctor v3.2.1" \
  --notes-file docs/CHANGELOG.md \
  --draft

# publish draft when ready
gh release edit v3.2.1 --repo luongnv89/mdoctor --draft=false
```
