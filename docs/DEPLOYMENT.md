...
5. Create/push release tag (e.g., `v3.2.0`) — the `Release` workflow
...
```bash
git tag -a v3.2.0 -m "mdoctor v3.2.0"
git push origin v3.2.0

# The Release workflow then creates the release titled "mdoctor v3.2.0"
# with docs/CHANGELOG.md as the notes. Manual equivalent (fallback only):
gh release create v3.2.0 \
  --repo luongnv89/mdoctor \
  --title "mdoctor v3.2.0" \
  --notes-file docs/CHANGELOG.md \
  --draft

# publish draft when ready
gh release edit v3.2.0 --repo luongnv89/mdoctor --draft=false
```
