...
check_python() {
  ...
  if command -v python3 >/dev/null 2>&1; then
    status_ok "Python3: $(tcap_or "$MDOCTOR_CMD_TIMEOUT_S" "unknown" python3 --version)"   # timeout-capped probe
    has_py3=true
  else
  ...
  if command -v pip3 >/dev/null 2>&1; then
    status_ok "pip3: $(tcap_or "$MDOCTOR_CMD_TIMEOUT_S" "unknown" pip3 --version)"

    # pip3 check (dependency issues) — timeout-capped; served by the
    # parallel prefetch when doctor.sh warmed it, same cap inline (#101).
    local pip_check_log _pc_rc=0
    pip_check_log="$(mdoctor_mktemp_file pip3-check)"
    perf_probe_capture pip3_check "$pip_check_log" || _pc_rc=$?
    if [ "$_pc_rc" -eq 0 ]; then
      status_ok "pip3 check passed (no dependency issues detected in current environment)."
    elif [ "$_pc_rc" -eq 124 ]; then
      status_info "pip3 check timed out (timeout ${MDOCTOR_REGISTRY_TIMEOUT_S}s) — skipped."
    else
    ...
    # pip3 outdated – full list to file (timeout-capped probe)
    local pip_out_file _po_rc=0
    pip_out_file="$(mdoctor_mktemp_file pip3-outdated)"
    perf_probe_capture pip3_outdated "$pip_out_file" || _po_rc=$?
    if [ "$_po_rc" -eq 124 ]; then
      status_info "pip3 list --outdated timed out (timeout ${MDOCTOR_REGISTRY_TIMEOUT_S}s) — skipped."
    else
      local pip_count
