...
source "${SCRIPT_DIR}/lib/perf_probes.sh"
source "${SCRIPT_DIR}/lib/json.sh"
source "${SCRIPT_DIR}/lib/metadata.sh"
source "${SCRIPT_DIR}/lib/registry.sh"
source "${SCRIPT_DIR}/lib/history.sh"

# Module context contract (Task 9.1): all check modules read these globals.
...
main() {
  ...
  if [ "$json_mode" = true ]; then
    ...
    # The return below skips main's tail: close the markdown report here
    # so the artifact stays complete in JSON mode too (e2e report test).
    md_append ""
